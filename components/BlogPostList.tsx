        ...
        {posts.map((post) => (
          <li key={post.slug}>
            <Link href={`/blog/${post.slug}`} className="card-calculator group block p-4">
              ...
              <p className="mt-2 line-clamp-2 text-sm font-semibold leading-snug">{post.title}</p>
              <span className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-text-muted group-hover:text-text">
                Читать →
              </span>
            </Link>
          </li>
    ...
    <ul className="divide-y divide-border border-y border-border">
      {posts.map((post) => (
        <li key={post.slug}>
          <Link
            href={`/blog/${post.slug}`}
            className="group block py-6 transition-colors hover:bg-surface first:pt-6"
          >
            <p className="font-mono text-xs text-text-subtle">{post.date}</p>
            <p className="mt-2 font-display text-xl font-semibold leading-snug group-hover:text-text">
              {post.title}
            </p>
